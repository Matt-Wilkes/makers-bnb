...
# Testing list all spaces, filtered
def test_view_my_spaces(page, test_web_address, db_connection):
    db_connection.seed('seeds/users_seed.sql')
    page.goto(f"http://{test_web_address}/login")
    ...
    #click link inside home.html
    page.click('._my_spaces')

    #checking content of view-my-spaces.html
    expect(page.locator('h1')).to_have_text('My Spaces')
    expect(page.locator('._name')).to_have_text(['Makers Mansion', 'Makers Shed', 'Makers Villa'])
    expect(page.locator('._name')).not_to_have_text(['Makers Retreat', 'Makers All-Inclusive', 'Makers Party'])
    expect(page.locator('.btn-secondary')).to_have_text('Create Space')
